...
import { GAME_FORGE_ID } from "@/lib/forge-specialists";
...
export function GameForgeSandboxPanel({
  initialFilePath,
  initialFileContent,
}: {
  ...
  const status = trpc.gameDevSandbox.getStatus.useQuery();
  const projects = trpc.gameDevSandbox.listProjects.useQuery();
  const createProject = trpc.gameDevSandbox.createProject.useMutation({
    onSuccess: () => {
      void utils.gameDevSandbox.listProjects.invalidate();
      void utils.gameDevSandbox.getStatus.invalidate();
    },
  });
  const saveFile = trpc.gameDevSandbox.saveFile.useMutation({
    onSuccess: () => {
      void utils.gameDevSandbox.listProjects.invalidate();
      void utils.gameDevSandbox.getStatus.invalidate();
    },
  });
  const runTest = trpc.gameDevSandbox.runTest.useMutation();
  const executeBuild = trpc.gameDevSandbox.executeBuild.useMutation();
  const createCheckout = trpc.gameDevSandbox.createUpgradeCheckout.useMutation();
  const confirmPayment = trpc.gameDevSandbox.confirmUpgradePayment.useMutation({
    onSuccess: () => void utils.gameDevSandbox.getStatus.invalidate(),
  });
  const upgradeTier = trpc.gameDevSandbox.upgradeTier.useMutation({
    onSuccess: () => void utils.gameDevSandbox.getStatus.invalidate(),
  });

  const [projectName, setProjectName] = useState("My Game");
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(null);
  const [filePath, setFilePath] = useState(initialFilePath ?? "scripts/Player.gd");
  ...
  const handleCreateProject = () => {
    createProject.mutate(
      { name: projectName, engine: "Godot 4" },
      {
  ...
  const handleExecuteBuild = () => {
    ...
    executeBuild.mutate(
      ...
      {
        onSuccess: (report) => {
          setLastBundleKb(Math.round((report.estimatedAssetMb ?? 0) * 1024));
          const lines = [
            report.success ? "✅ Build simulation passed" : "⚠️ Build issues found",
            `Files: ${report.filesChecked} · Est. assets: ${report.estimatedAssetMb} MB · Secured: ${report.secured ? "yes" : "review"}`,
          ];
    ...
    <ScrollView style={styles.root} contentContainerStyle={{ padding: 12, gap: 12 }}>
        ...
        <>
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <TextInput
              ...
              placeholder="src/scripts/Player.gd"
              placeholderTextColor={colors.muted}
              style={[styles.input, { color: colors.foreground, borderColor: colors.border }]}
            />
            <TextInput
      ...
      <View style={[styles.card, { backgroundColor: `${colors.primary}10`, borderColor: colors.primary }]}>
        ...
        <Text style={{ color: colors.muted, fontSize: 12, marginTop: 6, lineHeight: 18 }}>
          Chat · Learn · Hive · Web search · Voice · 3D workspace · Agent · Templates · GitHub · Preview · CI —
          ship simple games to Apple App Store & Google Play · secure sandbox up to 25 GB for massive games.
        </Text>
      </View>
    </ScrollView>
  );
