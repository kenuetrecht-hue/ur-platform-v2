...
export type TechBuilderLearnPanelProps = {
  creatorId: string;
  creatorName: string;
...
export function TechBuilderLearnPanel({
  creatorId,
  creatorName,
  onTryInSandbox,
}: TechBuilderLearnPanelProps) {
  ...
  useEffect(() => {
    const intro = `Welcome to **Learn to Code** with ${creatorName}.\n\nYou can learn to build software **on your own** — I guide you step by step, then you practice in the **Build** tab.\n\nPick a module below or ask anything. Level: **${level}**.`;
          ...
          {practiceQuestions.data.questions.map((q) => (
            <View key={q.id} style={[styles.exerciseCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              {"starterFile" in q && q.starterFile && onTryInSandbox ? (
                <Pressable
                  onPress={() => onTryInSandbox(q.starterFile!)}
